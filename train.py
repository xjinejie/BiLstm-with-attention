...
def train_evaluate(config, train_loader, val_loader,  vocab_size, verbose=True):
    model = BiLSTMAttnModel(vocab_size, config).to(config.device)
...
if __name__ == "__main__":
    ...
    for exp_name, changes in experiments.items():
        ...
        print(f"\n运行实验: [{exp_name}]")
        model_state, acc, history = train_evaluate(current_config, train_loader, val_loader, len(vocab))
